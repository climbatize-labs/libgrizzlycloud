...
#ifdef POOL_DEBUG
    hm_log(LOG_TRACE, pool->log, "{Pool}: returning node: %p/%d, new freenode: %p from pool: %p/%d", node, realsize, pool->freenode, pool, pool->size);
#endif
...
#ifdef POOL_DEBUG
    hm_log(LOG_TRACE, pool->log, "{Pool}: creating new pool with parent pool: %p/%d", p, p->size);
#endif
