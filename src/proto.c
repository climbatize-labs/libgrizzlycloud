...
static int add_intern(sn *dst, const void *src, const int nsrc)
{
    ...
    swap_memory(dst->s + offset_0, sizeof(nsrc));
...
{
    //swap_memory((void *)&v, sizeof(v));
    //return add_intern(dst, &v, sizeof(v));
    ...
    swap_memory(dst->s + offset_0, sizeof(v));
...
static int get_int_intern(sn *src, int *value)
{
    ...
    *value = *(int *)(src->s + src->offset);
    swap_memory((void*)value, sizeof(*value));
...
static int get_intern(sn *dst, sn *src)
{
    ...
    swap_memory((void*)&dst->n, sizeof(dst->n));
...
static int get_enum(sn *src, enum proto_e *value)
{
    ...
    int ret;
    ret = get_intern(&dst, src);
    if(ret != GCPROTO_OK) return ret;

    if(dst.n != sizeof(int)) return GCPROTO_ERR;

    *value = *(enum proto_e *)dst.s;
    */
...
{
    /*
	struct json_object *jobj, *payload;
	...
	json_object_object_add(jobj, "version", json_object_new_int(PROTO_VERSION));
	json_object_object_add(jobj, "payload", payload);
    */
...
int deserialize(struct proto_s *dst, sn *src)
{
	...
	enum json_type type;
	type = json_object_get_type(version);
	if(type != json_type_int) {
		return -1;
	}

	if(json_object_get_int(version) != GCPROTO_VERSION) {
		return -1;
	}

	struct json_object *command;
	json_object_object_get_ex(*jobj, "command", &command);
	type = json_object_get_type(command);
	...
	json_object_object_get_ex(*jobj, "payload", &payload);
	type = json_object_get_type(payload);
